...
class DeepgramASRAdapter(BaseASRAdapter):
    def __init__(self, config: ASRProviderConfig | None = None) -> None:
        self.config = config or ASRProviderConfig(provider="deepgram")
        self.started = False
        self.closed = False
    ...
    async def start_stream(self, session_id: str) -> None:
        if not self.config.is_configured():
            self.last_error = "Deepgram ASR config is incomplete"
            raise ValueError(self.last_error)
        self.started = True
        raise NotImplementedError("Deepgram ASR streaming is not implemented yet")
